...
class BlockTables:
    ...
    def append_block_ids(
        ...
        for i in range(self.num_kv_cache_groups):
            # Capture the slot previous occupant extent before overwriting.
            # overwrite=True (new request on a reused slot) writes only
            # [0, end); without clearing the tail, the predecessor block
            # ids linger in the row (observed: an image request leaves its
            # multi-block group entries behind and the next text request
            # inherits them). Any consumer that walks the per-group table
            # beyond the current request extent then reads the previous
            # request caches -- image-flavored KV after an mm request,
            # producing the deterministic EOS corruption of the next text
            # request. Zero the stale tail.
            ...
            if overwrite and end < prev_end:
                # Direct np+gpu writes: the staging buffers have fixed
                # capacities and reject the extra zero entries.
                self.block_tables[i].gpu[req_index, end:prev_end] = 0
