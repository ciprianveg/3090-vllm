...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            # Transition repair: a request that reaches decode with fresh
            # draft tokens but no pending bonus row (num_new == len(drafts),
            # i.e. num_computed caught up to num_tokens) schedules only the
            # draft rows. The model runner requires
            # scheduled >= drafts + num_bonus_tokens, else it asserts.
            # Grant the bonus row(s) here; allocate_slots runs after this,
            ...
            if (
                ...
                num_new_tokens += self.num_sampled_tokens_per_step
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # PR #52452 backport: validate the accepted speculative block
            # against the grammar before committing it to request history.
            if (
                len(new_token_ids) > 1
