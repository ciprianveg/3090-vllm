...
class SchedulerOutput:
    ...
    def make_empty(cls) -> "SchedulerOutput":
        return cls(
            ...
            scheduled_mm_req_ids=set(),
            num_common_prefix_blocks=[],
            finished_req_ids=set(),
            free_encoder_mm_hashes=[],
        )

    # req_ids of scheduled requests that carry encoder (multimodal) inputs,
    # including decode-only steps of such requests. Used by the speculator
    # to exclude their rows from draft context-KV precompute.
    scheduled_mm_req_ids: set[str] = field(default_factory=set)
