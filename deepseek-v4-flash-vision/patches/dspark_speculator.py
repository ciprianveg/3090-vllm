...
class DSparkSpeculator(DFlashSpeculator):
    ...
    def load_model(self, target_model: torch.nn.Module) -> None:
        # Text-only drafting for DSpark (upstream vLLM PR #33437 semantics).
        # DSpark drafts from target aux hidden states; drafted tokens are
        # always plain vocab ids (image sentinel ids exist only in prompts),
        # so the draft never needs multimodal embeddings. The base class
        # would otherwise mark us mm-capable because the draft aliases the
        # target embedding table, and the resulting per-step mm gather
        # `_gather_mm_embeddings` mutates encoder/mm state from the draft
        # path (upstream issue #43832) and corrupts subsequent requests.
