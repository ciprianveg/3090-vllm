...
# Keyed by (device, workspace lane): the draft (DSpark) and target run under
# different workspace lanes exactly so their scratch never overlaps — the
# previous device-only key made both models share one 128 MB buffer, letting
# draft CUDA-graph replays race the target's aux-stream sparse-index work.
_flashinfer_dsv4_workspace_by_device: dict[tuple[torch.device, int], torch.Tensor] = {}


def _get_flashinfer_dsv4_workspace(device: torch.device) -> torch.Tensor:
    from vllm.v1.worker.workspace import _workspace_lane

    key = (device, _workspace_lane.get())
    workspace = _flashinfer_dsv4_workspace_by_device.get(key)
    if workspace is None:
        ...
        _flashinfer_dsv4_workspace_by_device[key] = workspace
    return workspace
