...
class DeepseekV4VisionAttention(nn.Module):
    ...
    def forward(
        ...
        # Large images produce ~4k ViT patches; torch SDPA picks the math
        # backend here (torch 2.13 native registry ignores the sdpa_kernel
        # context manager on this build) and materializes an O(S^2)
        # attention matrix (~536 MB), OOMing the PP0 GPUs. Route the
        # bidirectional ViT attention through FlashInfer instead: O(S)
        # memory, same math. q/k/v are [S, H, D] here; FlashInfer returns
        # [S, H, D] directly.
        try:
            import flashinfer

            o = flashinfer.single_prefill_with_kv_cache(
                q.contiguous(), k.contiguous(), v.contiguous(), causal=False
            )
