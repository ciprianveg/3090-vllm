    ...
    def sample_tokens(
        ...
        if self.speculator is not None:
            ...
            if _step_has_prefill:
                # mm x draft isolation: never run the drafter on a step
                # carrying multimodal rows. The DSpark draft backbone and
                # its context-KV precompute write draft-KV for every query
                # row; image-flavored draft-KV persists in the KV pool
                # after the mm request frees its blocks, and the NEXT
                # request's anchor position (whose context-KV is not
                # written until the following step's precompute) reads the
                # stale image KV, producing an EOS-led draft chain that
                # the probabilistic verify accepts (the deterministic
                # 2-token death of text-after-image). Skipping propose
                # keeps the pool text-clean. Fill -1 placeholders so the
                # verify auto-rejects and every request in the step
                # samples plainly (one unspeculated step).
                # Safe rejectable drafts: BOS repeats. Valid vocab ids
                # (the combine kernel writes draft ids raw into the
                # forward input, so -1 placeholders fault the embedding),
                # and p_target(BOS | mid-text) is ~0, so the verify never
                # accepts them and every request in the step samples
                # plainly.
                draft_tokens = torch.zeros_like(
                    self.req_states.draft_tokens[input_batch.idx_mapping]
                )
                # The probabilistic rejection sampler reads the speculator
                # cached draft_logits (per-slot, persistent). Skipping
                # propose leaves the previous owner values in place, and
                # the ratio test / residual recovery then run on garbage,
                # deterministically producing wrong recovery tokens (the
                # observed EOS). Claim certainty on the drafted BOS token
                # and impossibility elsewhere: acceptance probability
                # becomes p_target(BOS) (~0, rejected) and the recovery
                # residual collapses to the plain target distribution.
                _dl = getattr(self.speculator, "draft_logits", None)
                if _dl is not None:
                    _slots = input_batch.idx_mapping
                    _dl[_slots] = torch.finfo(_dl.dtype).min
                    _dl[_slots, :, 0] = 0.0
            else:
                with use_workspace_lane(self._draft_workspace_lane):
